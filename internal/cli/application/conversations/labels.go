...
var labelsListCmd = &cobra.Command{
	Use:   "list",
	Short: "List conversation labels",
	RunE:  runLabelsList,
}

var labelsSetCmd = &cobra.Command{
	Use:   "set",
	Short: "Set conversation labels",
	RunE:  runLabelsSet,
}

func init() {
	labelsListCmd.Flags().Int("id", 0, "Conversation ID")
	labelsListCmd.MarkFlagRequired("id")
	labelsCmd.AddCommand(labelsListCmd)

	labelsSetCmd.Flags().Int("id", 0, "Conversation ID")
...
func runLabelsList(cmd *cobra.Command, args []string) error {
	...
	labels, err := client.ListConversationLabels(ctx, id)
	if err != nil {
...
func runLabelsSet(cmd *cobra.Command, args []string) error {
	...
	result, err := client.SetConversationLabels(ctx, id, labels)
	if err != nil {
		return cmdutil.WriteError(cmd, contract.ErrCodeServer, err.Error())
	}
