...
func TestAgentBotsList(t *testing.T) {
	...
	Cmd.Root().SetArgs([]string{"application", "agent-bots", "list"})
...
func TestAgentBotsCreate(t *testing.T) {
	...
	Cmd.SetOut(&stdout)
	Cmd.SetErr(&bytes.Buffer{})
	Cmd.Root().SetArgs([]string{"application", "agent-bots", "create", "--name", "Helper Bot"})
	err := Cmd.Root().Execute()
	if err != nil {
		t.Fatalf("Execute error: %v", err)
	}

	if gotBody["name"] != "Helper Bot" {
		t.Errorf("body name = %v, want Helper Bot", gotBody["name"])
	}
